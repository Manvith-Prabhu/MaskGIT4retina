...
class MaskGIT(Trainer):
    ...
    def __init__(self, args):
        ...
        if self.args.test_only:
            ...
            self.sae = SampleAndEval(device=self.args.device, num_images=50)
    ...
    def get_network(self, archi):
        ...
        if archi == "vit":
            model = MaskTransformer(
                #img_size=self.args.img_size, hidden_dim=768, codebook_size=self.codebook_size, depth=24, heads=16, mlp_dim=3072, dropout=0.1     # Tiny
            ...
            # Load config
            config = OmegaConf.load(self.args.vqgan_folder + "/model_config.yaml")
    ...
    def get_mask_code(code, mode="arccos", value=None, codebook_size=256): 
        ...
        mode -> str: the rate of value to mask value -> int: mask the code by the value
    ...
    def train_one_epoch(self, log_iter=2500):
        ...
        for x, y in bar:
            ...
            masked_code, mask, min_prob, min_prob_token, min_prob_index = self.get_mask_code(code, value=self.args.mask_value, codebook_size=self.codebook_size)
